...
/**
 * Node parser for XML -> Sphinx xmlpipe2 conversion
 * @author yokochi
 */
public class PgSchemaNode2SphDs extends PgSchemaNodeParser {
	...
	public void invokeRootNestedNode() throws ParserConfigurationException, TransformerException, IOException {
		...
		for (int n = 0; n < nested_fields; n++)
			schema.parseChildNode2SphDs(proc_node, table, schema.getTable(nested_table_id[n]), key_name, nested_key_name[n], list_holder[n], table.bridge, 0);
	...
	public void invokeChildNestedNode(PgSchemaNodeTester node_test) throws ParserConfigurationException, TransformerException, IOException {
		...
		for (int n = 0; n < nested_fields; n++) {
			...
			schema.parseChildNode2SphDs(exists || nested ? node_test.proc_node : proc_node, table, nested_table, node_test.key_name, nested_key_name[n], list_holder[n], !exists, exists ? 0 : node_test.key_id);
	...
	public void invokeChildNestedNode() throws ParserConfigurationException, TransformerException, IOException {
		...
		for (int n = 0; n < nested_fields; n++) {
			...
			if (existsNestedNode(schema, nested_table, proc_node))
				schema.parseChildNode2SphDs(proc_node, table, nested_table, key_name, nested_key_name[n], list_holder[n], false, 0);
