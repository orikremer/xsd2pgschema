...
public class XmlBuilder extends CommonBuilder {
	 ...
	 */	
	public XmlBuilderNestTester nestChildNode2Xml(final PgSchema schema, final PgTable table, final Object parent_key, final boolean as_attr, XmlBuilderNestTester parent_nest_test) throws PgSchemaException {
				...
				if (table.has_attrs) {
					...
					for (PgField field : fields) {
						...
						if (field.attribute) {
							...
							if (key != null)
								nest_test.merge(nestChildNode2Xml(schema, schema.getTable(field.foreign_table_id), key, true, nest_test));
						...
						if (field.nested_key && !field.nested_key_as_attr) {
							...
							if (key != null) {
								...
								if (nested_table.content_holder || !nested_table.bridge || as_attr)
									nest_test.merge(nestChildNode2Xml(schema, nested_table, key, false, nest_test));

								// skip bridge table for acceleration

								else if (nested_table.list_holder)
									nest_test.merge(skipListAndBridgeNode2Xml(schema, nested_table, key, nest_test));

								else
									nest_test.merge(skipBridgeNode2Xml(schema, nested_table, key, nest_test));
	 ...
	 * @throws PgSchemaException the pg schema exception
	 */	
	public XmlBuilderNestTester skipListAndBridgeNode2Xml(final PgSchema schema, final PgTable table, final Object parent_key, XmlBuilderNestTester parent_nest_test) throws PgSchemaException {
	 ...
	 * @throws PgSchemaException the pg schema exception
	 */	
	public XmlBuilderNestTester skipBridgeNode2Xml(final PgSchema schema, final PgTable table, final Object parent_key, XmlBuilderNestTester parent_nest_test) throws PgSchemaException {
		...
		try {
			...
			for (PgField field : table.nested_fields) {
				...
				if (!field.nested_key_as_attr) {
					...
					if (nested_table.content_holder || !nested_table.bridge)
						nest_test.merge(nestChildNode2Xml(schema, nested_table, parent_key, false, nest_test));

					// skip bridge table for acceleration

					else if (nested_table.list_holder)
						nest_test.merge(skipListAndBridgeNode2Xml(schema, nested_table, parent_key, nest_test));

					else
						nest_test.merge(skipBridgeNode2Xml(schema, nested_table, parent_key, nest_test));
