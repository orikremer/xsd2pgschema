...
public class tsv2pgsql {
	...
	public static void main(String[] args) {
		...
		try {
			...
			PgSchemaClientImpl client = new PgSchemaClientImpl(is, option, fst_conf, client_type, MethodHandles.lookup().lookupClass().getName(), null, true);
	...
	private static void showUsage() {
		...
		System.err.println("        --doc-key-if-no-inplace (append document key if no in-place document key, set --no-doc-key options by default)");
